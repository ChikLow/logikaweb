...
class BlogDB():
    ...
    def get_all_posts_by_category(self,category_id):
        self.open()
    ...
    def get_post(self,post_id):
        self.open()
        self.cursor.execute("SELECT * FROM posts WHERE category_id=?", [post_id])
    ...
    def create_post(self, title, text, category,image_name):
        ...
        self.cursor.execute("INSERT INTO posts(title, text, category_id, image) VALUES(?,?,?,?)", [title, text, category_db[0], image_name])
        self.conn.commit()
        self.close()